...
$(document).on('click','.add',function(){
   ...
   $('.save').show();
      ...
      $('[name="no_meja"]').removeAttr('disabled');
  tambahBahan();
});
...
$(document).on('click','.update',function(){
    ...
    $('.transaksi-row').remove();
    $.post("pemesanan.php?a=edit", {idx: idx}).done(function( data1 ) {
      ...
      $.post("pemesanan.php?a=edit_detail", {idx: json.id_pemesanan}).done(function( data ) {
    // var json = $.parseJSON(data1.detail);
        var json = $.parseJSON(data);
        for (var i = 0; i < json.length; i++) {
             $('#tableBahan').append(
            '<tr class="transaksi-row"><td><select class="select2 form-control" style="width:100%" name="menu[]"><option value="">select menu</option>'+menu+'</select></td>'+
            '<td><input type="text" class="form-control" name="jumlah[]" value="" required><input type="hidden" class="form-control" value="'+json[i].id_detail_pemesanan+'" name="id_detail_pemesanan[]"></td>'+
            '<td><input type="text" class="form-control" name="harga[]" disabled value="" required></td>'+
            '<td><input type="text" class="form-control" name="subtotal[]" disabled value="" required></td>'+
            '<td>'+
...
$(document).on('click','.view',function(){
  $('.save').hide();
    ...
    $.post("pemesanan.php?a=edit", {idx: idx}).done(function( data1 ) {
      ...
      $('[name="id_pemesanan"]').val(json.id_pemesanan);
      $('[name="no_meja"]').val(json.no_meja);
      $('[name="no_meja"]').attr('disabled','disabled');
      $('[name="id_pegawai"]').val(json.id_pegawai);
      $.post("pemesanan.php?a=edit_detail", {idx: json.id_pemesanan}).done(function( data ) {
    // var json = $.parseJSON(data1.detail);
        var json = $.parseJSON(data);
        for (var i = 0; i < json.length; i++) {
             $('#tableBahan').append(
            '<tr class="transaksi-row"><td><select disabled class="select2 form-control" style="width:100%" name="menu[]"><option value="">select menu</option>'+menu+'</select></td>'+
            '<td><input type="text" class="form-control" disabled name="jumlah[]" value="" required><input type="hidden" class="form-control" value="'+json[i].id_detail_pemesanan+'" name="id_detail_pemesanan[]"></td>'+
            '<td><input type="text" class="form-control" name="harga[]" disabled value="" required></td>'+
            '<td><input type="text" class="form-control" name="subtotal[]" disabled value="" required></td>'+
            '<td>'+
...
$(document).on('click','.delete',function(){
    ...
    bootbox.confirm("Apakan anda yakin akan menghapus "+$(tr).find('td').eq(1).html()+"?", function(result){
        if (result) {
            $.post("pemesanan.php?a=delete", {idx : idx}).done(function( data ) {
                if (data == '1'){
...
function get_menu(){
  var list="";
  $.ajax({ type:'POST',url:"pemesanan.php?a=get_menu"}).done(function( data ) {
    var json = $.parseJSON(data);
...
function hapusBahan(el){
  var idx = $(el).closest('tr').find('[name="id_detail_pemesanan[]"]').val();
  // var kode = $(el).closest('tr').find('[name="menu[]"]').val();
  if(idx == 0){
    $(el).parent().parent().remove();
  }else{
    bootbox.confirm("Apakan anda yakin akan menghapus menu?", function(result){
      if (result) {
        $.post("pemesanan.php?a=delete_detail", {idx : idx}).done(function( data ) {
          if (data == '1'){
            bootbox.alert("Data berhasil dihapus.", function(){
              $(el).parent().parent().remove();
            })
...
$(document).on('change','[name="menu[]"]',function(){
  ...
  if(id){
    $.post("pemesanan.php?a=get_menu_details", {id: id}).done(function( data ) {
      var json = $.parseJSON(data);
      // console.log(json)
...
function tambahBahan(){
  ...
  $('#tableBahan').append(
    '<tr class="transaksi-row"><td><select class="select2 form-control" style="width:100%" name="menu[]"><option value="">select menu</option>'+menu+'</select></td>'+
    '<td><input type="text" class="form-control" name="jumlah[]" required><input type="hidden" class="form-control" value="0" name="id_detail_pemesanan[]"></td>'+
    '<td><input type="text" class="form-control" name="harga[]" disabled value="" required></td>'+
            '<td><input type="text" class="form-control" name="subtotal[]" disabled value="" required></td>'+
    '<td>'+
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="table-responsive">
                            <table name="asd" class='table table-bordered table-hover table-striped' width="100%" id="dataTables-export">
                                ...
                                <?php
                                    $i = 1;
                                    while($a=mysqli_fetch_array($pemesanan)){
                                ?>
                                    <tr idx='<?=$a['id_pemesanan']?>'><td><?=$i?></td><td><?=$a['id_pemesanan']?><td><?=$a['tgl_pesan']?></td><td><?=$a['no_meja']?></td><td>
                                      <?php
                                      if($a['status']==1 && $_SESSION['jabatan'] != 6){
                                      ?>
                                      ...
                                      ?>
                                       ...
                                       if($_SESSION['jabatan']==6){
                                        ?>
                                        <a href="pemesanan.php?a=ganti_status&b=<?=$a['id_pemesanan']?>" class="view btn btn-sm btn-white text-black"><i class="fa fa-check fa-lg"></i></a> 
                                        <?php
                                        }
                                        ?> 
      ...
      <div class="modal-dialog modal-lg">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body" id="print">
            <form name="form-<?php echo $title; ?>" id="myForm" method="POST" action="../public_html/pemesanan.php?a=save_data" class=" form-horizontal">
            <div class="row">
                <div class="form-group required" style="margin-bottom: 20px">
                  <label class="col-sm-3 control-label">No Meja</label>
                  <div class="col-sm-8">
                  <input type="text" class="form-control" maxlength="50" required name="no_meja" placeholder="No Meja">
                ...
                <div class="col-lg-12">
                  <table class="table table-striped table-bordered" id="tableBahan" width="100%">
                    <tr><td width="40%">Menu</td><td width="10%">Jumlah</td><td width="20%">Harga</td><td>Sub Total</td><td></td></tr>
                    
                  </table>
                  <!-- <label class="col-sm-3 control-label pull-right">Total Bayar</label> -->
                  <div class="col-sm-4 pull-right">
                    <input type="text" class="form-control " maxlength="50" required disabled name="jml_bayar" >
                  </div>
                </div>
              </div>
            </div>
